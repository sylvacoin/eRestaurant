...
class Role extends MX_Controller
{
    ...
    public function submit_data()
    {
        ...
        $count = $this->mdl_role->get()->num_rows();
...
//            }
    ...
    public function get_priv($priv)
    {
        $privileges = [
            ...
            'cs_sms'   => 'can send sms',
            'cm_page'  => 'can manage pages',
            'cm_tasks' => 'can manage tasks',
            'cm_biz'   => 'can manage businesses',
            'cm_loc'   => 'can manage locations',
            'cm_pay'   => 'can manage payments',
            'cm_conf'  => 'can manage configurations',
        ];
    ...
    public function _set_default($id, $defaultPostName = 'default')
    {
        if ($this->input->post($defaultPostName) == 1 || $defaultPostName == 1) {
            ...
            $this->db->update('roles', ['is_default' => null]);

            $this->db->where('id', $id);
            return $this->db->update('roles', ['is_default' => 1]);
        }
    }
    ...
    {
        $this->db->where('id', $id);
        $role = $this->mdl_role->get()->row();
        return in_array($action, unserialize($role->priv));
    }

    public function get_role_privilege($id)
    {
        $role = $this->_get_where($id)->row();
        return unserialize($role->priv);
    }

    public function get_role_name($id)
    {
        $role = $this->_get_where($id);
