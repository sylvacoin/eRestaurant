	    ...
	    <?php if ($roles->num_rows() > 0): foreach ( $roles->result() as $row ): ?>
			...
			<td>
				...
				<?php if ( $row->is_deletable ): ?>
	                        <a href="<?= site_url('role/delete/'.$row->id) ?>" class="btn btn-danger" type="button"><i class="fa fa-trash"></i> Delete</a>
				<a href="<?= site_url('role/edit/'.$row->id) ?>" class="btn btn-default" type="button"><i class="fa fa-pencil"></i> Edit</a>
				<?php endif; ?>
				<?php if ( $row->is_default === NULL): ?>
				<a href="<?= site_url('role/default/'.$row->id) ?>" class="btn btn-default" type="button"><i class="fa fa-bullseye"></i> Make default</a>
				<?php endif; ?>
			    </div>
